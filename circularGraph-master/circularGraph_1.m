classdef circularGraph_1 < handle
...
% Copyright 2016 The MathWorks, Inc.
  ...
  methods
    function this = circularGraph_1(adjacencyMatrix,varargin)
      % Constructor
      p = inputParser;
      [row,col,v] = find(adjacencyMatrix);
      val = [];
      for i = 1:length(row)
        val(end+1) = adjacencyMatrix(row(i),col(i));
      end
      num = unique(val);
      
      defaultColorMap = parula(length(num));%
      ...
      for i = 1:length(v)
        [l,r_,v_] = find(num==adjacencyMatrix(row(i),col(i)));
        if row(i) ~= col(i)
