classdef circularGraph_2 < handle
...
% Copyright 2016 The MathWorks, Inc.
  ...
  methods
    function this = circularGraph_2(adjacencyMatrix,varargin)
      % Constructor
      p = inputParser;
      region = [9,11,5,4,4,4,4,5,11,9];
      region_idx = load('region_idx.mat');
      region_idx = region_idx.region_idx;
      defaultColorMap = parula(5);%
      ...
      for i = 1:length(adjacencyMatrix) 
        ...
        this.Node(i).Color = this.ColorMap(region_idx(i),:);
        ...
        if row(i) ~= col(i)
          if abs(row(i) - col(i)) - length(adjacencyMatrix)/2 == 0 
            ...
            this.Node(row(i)).Connection(end+1) = line(...
              ...
              'Color', this.ColorMap(region_idx(row(i)),:),...
              'PickableParts','none');
